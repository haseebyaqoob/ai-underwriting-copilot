...
import { useAttachFromWallet, useCreateApplication, useDocumentQueue, useEvidenceWallet, useSubmitApplication, useUploadDocument } from "@/lib/yaqeen-queries";
import { UploadCloud, CheckCircle2, Wallet, Loader2, Plus } from "lucide-react";
      ...
      <div className="paper-card p-6">
        ...
        {step === 2 && applicationId && (
          <div className="space-y-4">
            ...
            <WalletSuggestions applicationId={applicationId} />
...
/**
 * Evidence Wallet redesign: "when creating a new application, automatically
 * suggest reusable evidence" (product brief). Shows the applicant's wallet
 * right at the top of the evidence step of the new-application wizard,
 * with a one-tap attach per item -- reuses the same
 * /applicant/evidence/wallet/attach endpoint the full Evidence page uses,
 * so a document attached here shows up identically everywhere else.
 */
function WalletSuggestions({ applicationId }: { applicationId: string }) {
  const { data: wallet, isLoading } = useEvidenceWallet();
  const attach = useAttachFromWallet();
  const [attachedIds, setAttachedIds] = useState<Set<string>>(new Set());

  if (isLoading || !wallet || wallet.length === 0) return null;

  return (
    <div className="rounded-lg border border-navy/20 bg-navy/[0.03] p-4">
      <div className="mb-2 flex items-center gap-2 text-sm font-medium text-foreground">
        <Wallet className="h-4 w-4 text-navy" /> Reuse from your Evidence Wallet
      </div>
      <p className="mb-3 text-xs text-muted-foreground">
        These documents were verified on a previous application. Attach them here instead of uploading again.
      </p>
      <ul className="space-y-2">
        {wallet.map((item) => {
          const attached = attachedIds.has(item.id);
          return (
            <li key={item.id} className="flex items-center justify-between gap-3 rounded-lg border border-border/70 bg-card/60 px-3 py-2">
              <span className="truncate text-sm text-foreground">{item.label}</span>
              <button
                disabled={attach.isPending || attached}
                onClick={() =>
                  attach.mutate(
                    { applicationId, walletItemId: item.id },
                    { onSuccess: () => setAttachedIds((prev) => new Set(prev).add(item.id)) }
                  )
                }
                className="flex shrink-0 items-center gap-1 rounded-lg border border-navy/30 px-2.5 py-1.5 text-xs text-navy hover:bg-navy/5 disabled:opacity-50"
              >
                {attached ? <CheckCircle2 className="h-3 w-3" /> : attach.isPending ? <Loader2 className="h-3 w-3 animate-spin" /> : <Plus className="h-3 w-3" />}
