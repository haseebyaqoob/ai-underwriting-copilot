...
import {
  countWalletReuseMatches,
  RequestedDocumentsBanner,
  WalletReuseBanner,
} from "@/components/yaqeen/evidence";
import { EvidenceWizard } from "@/components/yaqeen/evidence-wizard";
import {
...
function EvidencePage() {
  ...
  const { data: checklist, isLoading: checklistLoading } = useEvidenceChecklist(
  ...
  // Evidence Wallet discoverability (design decision, this session): a
  // top-of-page banner surfacing how many saved documents can be reused
  // on THIS application, before the user starts scrolling through
  // categories. Kept in lockstep with the per-item "Use from Wallet"
  // button via the shared countWalletReuseMatches helper.
  const walletReuseCount = useMemo(
    () => countWalletReuseMatches(checklist, walletBySubtype),
    [checklist, walletBySubtype],
  );
  ...
  return (
    <div className="space-y-8">
      <SectionHeader
        ...
        sub="A few short steps — identity, business documents, digital transactions, and photos. Upload what you have, reuse what's already in your wallet, and review everything at the end. Nothing here needs to be perfect on the first try."
      />
      ...
      {appsLoading ? (
        ...
        <div className="paper-card p-6 text-sm text-muted-foreground">
          ...
          <a href="/applicant/applications/new" className="underline underline-offset-4">
            start one
          </a>{" "}
        ...
        <label className="block max-w-md">
          ...
          <select
            ...
            className="w-full rounded-lg border border-input bg-card/70 px-3 py-2.5 text-sm"
          >
            {apps.map((a) => (
      ...
      {activeAppId && !checklistLoading && <WalletReuseBanner count={walletReuseCount} />}

      {activeAppId && (
        <>
          {checklist && checklist.openRequests.length > 0 && (
            <RequestedDocumentsBanner requests={checklist.openRequests} />
          )}
          {checklistLoading || !checklist ? (
            <div className="space-y-4">
              <div className="paper-card h-12 animate-pulse" />
            ...
            <EvidenceWizard
              ...
              attachingId={attachingId ?? undefined}
